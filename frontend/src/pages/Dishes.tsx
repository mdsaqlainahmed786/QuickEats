    ...
    const fetchDishes = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/api/v1/dishes/get-all-dishes"
        );
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {filteredDishes.map((dish) => (
          <div
            key={dish.id}
            className="bg-white shadow-md rounded-lg cursor-pointer p-4 text-center hover:shadow-xl hover:border hover:border-red-500"
          >
            ...
            <p className="text-2xl font-semibold">${dish.price}</p>
